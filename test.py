"""
Декоратор
def cached(key_builder: Callable, ttl: int=300):
    def decorator(func):
        @wraps(func)
        async def wrapper(depends: RouterDepends = Depends(), **kw):
            if not depends.request:
                raise Exception("Request not provided")
            
            redis = depends.request.app.state.redis
            if not redis:
                raise Exception("Redis instance not provided")
            
            cache_key = key_builder(depends=depends)
            ...
            result = await func(depends=depends, **kw)
...
class CachedAPIRouter(APIRouter):
    def add_api_route(self, path: str, endpoint: Callable, **kwargs):
        ...
        @wraps(endpoint)
        async def wrapped_endpoint(*args, **kw):
            return await endpoint(*args, **kw)
        
        cached_endpoint = cached(ttl=get_sunday_midnight_ttl(), key_builder=key_builder)(wrapped_endpoint)
        super().add_api_route(path, cached_endpoint, **kwargs)
"""
